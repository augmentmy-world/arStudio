...
var addPlnToScene = function(pmmgroup, pln, plnp) {
	//add a new node
...
var PMMModule = {
	...
	pmm: function() {
		...
		that.ctx.translate(640, 0);
			that.ctx.scale(-1, 1);
				that.ctx.drawImage(that.video, 0, 0);
			that.ctx.scale(-1, 1);
		that.ctx.translate(-640, 0);
		...
		for (var i = 0; i < plns.length; i++) {
			...
			that.ctx.beginPath();
			that.ctx.moveTo(p[0][0], p[0][1]);
			that.ctx.lineTo(p[1][0], p[1][1]);
			that.ctx.lineTo(p[2][0], p[2][1]);
			that.ctx.lineTo(p[3][0], p[3][1]);
			that.ctx.closePath();
			that.ctx.fill();
...
/*
		...
		fsquad.coords = [[0, 0], [1, 0], [0, 1], [1, 1]];
		// Upload to GPU memory
		fsquad.upload();
		//the render texture
		var overlay = new GL.Texture(canvas.width, canvas.height);
		//the shader for the fullscreen quad
		var postprocess = new GL.Shader(
			'varying vec2 uv; \
			void main() { \
				uv = gl_TexCoord.xy; \
