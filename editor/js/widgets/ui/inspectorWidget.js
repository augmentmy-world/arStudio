...
InspectorWidget.prototype.showComponentsInterface = function (object, inspector) {
    ...
    if (object._missing_components && object._missing_components.length) {
        for (var i = 0; i < object._missing_components.length; ++i) {
            ...
            inspector.addStringButton("Component class", name, {
                name_width: 120, width: "90%", comp_info: comp_info, button_width: 80, button: "Convert", callback_button: function (v) {
            ...
            inspector.addButton(null, "<img src='imgs/mini-icon-trash.png'/>", {
                index: i, width: "10%", callback: function () {
                    CORE.userAction("node_changed", object);
...
InspectorWidget.prototype.inspectScene = function (scene) {
    ...
    inspector.on_refresh = function () {
        ...
        if (window.PlayModule)
            inspector.addStringButton("Test URL", scene.extra.test_url || "", {
                callback: function (v) { scene.extra.test_url = v; }, callback_button: function () {
                    PlayModule.launch();
                }
            });
        inspector.addSeparator();
        ...
        for (var i in scene.external_scripts) {
            inspector.addStringButton(null, scene.external_scripts[i], {
                index: i, callback: function (v) {
                    if (!v)
                        return;
        ...
        inspector.addStringButton(null, "", {
            callback: function (v) {
        ...
        for (var i in scene.global_scripts) {
            inspector.addScript(null, scene.global_scripts[i], {
                width: "100% - 60px", index: i, callback: function (v) {
                    if (!v) {
            ...
            inspector.addButton(null, "<img src='imgs/mini-icon-trash.png'/>", {
                width: 30, index: i, callback: function () {
                    scene.global_scripts.splice(this.options.index, 1);
                    inspector.refresh();
                }
            });
		...
		*/
        ...
        for (var i in scene.preloaded_resources) {
            inspector.addStringButton(null, i, {
                callback: function (v) {
...
InspectorWidget.prototype.inspectNode = function (node, component_to_focus) {
    ...
    inspector.on_refresh = (function () {
        ...
        if (node == LS.GlobalScene.root) //main node use an special editor
        {
            ...
            inspector.widgets_per_row = 2;
            inspector.addString("name", node._name || "", {
                name_width: 80, callback: function (v) {
                    if (!v)
                        return node._name;
            ...
            if (node.prefab) {
                inspector.widgets_per_row = 2;
                inspector.addStringButton("prefab", node.prefab, {
                    width: -30, name_width: 80, callback: function (v) {
                        node.prefab = v;
                        inspector.refresh();
                    }, callback_button: function (v, evt) {
                        EditorModule.showSelectResource({
                            type: "Prefab", on_complete: function (v) {
                                node.prefab = v;
                                inspector.refresh();
                            }
                        });
                ...
                inspector.addButton(null, LiteGUI.special_codes.navicon, {
                    height: "1em", width: 30, callback: function (v, evt) {
                        var menu = new LiteGUI.ContextMenu(["Choose Prefab", "Unlink prefab", "Reload from Prefab", "Update to Prefab"], {
                            title: "Prefab Menu", event: evt, callback: function (action) {
                ...
                inspector.addButton(null, "Go to Node", {
                    width: "25%", callback: function () {
            ...
            inspector.addLayers("layers", node.layers, {
                name_width: 80, pretitle: AnimationModule.getKeyframeCode(node, "layers"), callback: function (v) {
                    node.layers = v;
                    RenderModule.requestFrame();
                }
            });
            ...
            for (var i in EditorModule.node_editors) {
                // leo add to not shwo materials as component in inspector 2017-10-19 
                var isGeometricPrimitive =  node.hasComponent(LS.Components.GeometricPrimitive);
                var isMaterial =EditorModule.node_editors[i] == EditorModule.showMaterialNodeInfo;
                if(isMaterial && !isGeometricPrimitive)
                    continue;
