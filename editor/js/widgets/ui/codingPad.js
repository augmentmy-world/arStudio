...
//cwx removed so we get save button back	this.save_button.style.display = instance.fullpath ? null : "none";
	var filename = "";
	if(instance)
	{
...
{
	var that = this;
	var dialog = new LiteGUI.Dialog( { title:i18n.gettext("Select Code"), draggable: true, closable: true });
	
	var widgets = new LiteGUI.Inspector( { name_width: 100 } );

	if(!skip_create)
		widgets.addStringButton(i18n.gettext("New script"),"unnamed.js", { button:"GO", callback_button: function(v){
			var filename = v;
			if(!filename)
				return;
...
CodingPadWidget.prototype.createCodingArea = function( container )
{
	...
	top_widgets.addButton(null, LiteGUI.special_codes.navicon,{ callback: function(v,e) { 
	...
	*/
	...
	top_widgets.addButton(null,"?",{ callback: function(v) { 
		that.onShowHelp();
	}});
